...
| `dependabot/fetch-metadata` | `@v3` | Reads Dependabot PR metadata (`update-type`, `package-ecosystem`) for the auto-merge template. v3.1.0 also fixed `update-type` incorrectly returning `null` for some Composer/Terraform/pip PRs with non-standard commit prefixes |
...
- **`gradle-react-native-architectures` (default `arm64-v8a`) overwrites `reactNativeArchitectures` in the project's `gradle.properties`** — the workflow wins over whatever is checked in. A caller whose app ships to devices beyond arm64 (or whose `abiFilters` list more) **must** pass the full list explicitly, or CI silently narrows the AAB and Play rejects the release / breaks the upgrade path for existing non-arm64 installs. Dropping the input while leaving the explanatory comment behind is a real and easy-to-miss regression.
