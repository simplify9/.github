...
| Helm chart with Gateway API | ChartMuseum | [`gateway-chart-cicd.yml`](#gateway-chart-cicdyml) |
...
CI/CD for **Cilium Gateway API-aware** Helm charts: compute SemVer → `helm lint --strict` + routing/ConfigMap render assertions → package → push to ChartMuseum → tag origin. Routing modes (default / ingress / gateway / dual) and ConfigMap gating are validated by parsing the rendered manifests with `yq`, not by grepping raw text. The chart version is derived from existing git tags via [`determine-semver`](#versioning--determine-semver) and the successful run tags origin so the counter advances — it is **not** based on the run number.
...
| `major-version` | | `1` | SemVer major component |
| `minor-version` | | `0` | SemVer minor component |
| `update-dependencies` | | `true` | Run `helm package --dependency-update` |
...
**Required secrets:** `registry-username`, `registry-password` (`github-token` optional — used by the tag job; falls back to the built-in `GITHUB_TOKEN`)

**Outputs:** `version`, `chart-name`, `chart-package`, `chart-repo-url` (ChartMuseum upload endpoint — an upload API URL, not a fetch URL)

> **TODO — migrate to OCI.** ChartMuseum HTTP upload is the legacy distribution path. The next step is to publish via an **OCI registry** (`helm push chart.tgz oci://...`), as [`sw-cicd.yml`](#sw-cicdyml) already does (`chart-publish-method`). OCI gives immutable, digest-pinned, signable charts and removes the dependency on a standalone ChartMuseum service.
